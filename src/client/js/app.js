...
function setupSocket(socket) {
    ...
    socket.on("RIP", function (playerGotEaten, playerData, playerMass) {
        console.log(playerGotEaten, "Player who got eaten");
        console.log(playerData, "Remaining player data");
        ...
        const totalMass = playerData.reduce(
            (sum, player) => sum + player.massTotal,
            0
        );
        const fee = 5;
        const pool = playerData.length * fee;
        const reservedAmount = pool * 0.1;
        const distributablePool = pool - reservedAmount;
        ...
        render.drawErrorMessage(
            `Hi, ${
                playerGotEaten.name
            }! You have been eliminated.  total mass :${playerMass}. 
